# 👨🏻‍💻 Rohit Raj Srivastava  

🚀 **Frontend-Focused Full Stack Developer** | React • MERN • UI Systems  
🎯 Building real-world applications with scalable architecture & clean UI  
🌐 **Portfolio:** [rohit-raj-srivastava.vercel.app](https://rohit-raj-srivastava.vercel.app)
---

## 🧠 About Me  

- 🚀 Frontend Developer with strong focus on **React & Tailwind CSS**
- 🏥 Currently building **SwiftCare** — a full-scale **MERN healthcare system**
- 📊 Exploring **Data Analytics (SQL, Excel, Power BI)** alongside development
- ⚡ Passionate about **performance, clean architecture & real-world problem solving**
- 👯 Open to **collaborations** on React, MERN & system-based projects  
- 💬 Ask me about **React patterns, state management, UI architecture**  

---

## 🌐 Live Projects  

### 🔹 CashFlow (Expense Tracker PWA)  
👉 https://cashflow-c.vercel.app  
*React + Express + MongoDB • Offline support • Analytics dashboard • Data retention logic*  

### 🔹 Text Analyzer  
👉 https://text-analyzer-theta.vercel.app  
*Vanilla JS • Real-time text processing • Performance optimized (debounced logic)*  

### 🔹 Gifts Garden (E-Commerce)  
👉 https://giftsgarden.lennobyte.com  
*NestJS + React • Inventory • Transactions • API integration*  

### 🔹 Portfolio Website  
👉 https://rohit-raj-srivastava.vercel.app  
*Dynamic portfolio with admin dashboard & visitor tracking*  

---

## 💻 Tech Stack  

<p align="center">
  <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/HTML/html2.svg"/>
  <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/CSS/css2.svg"/>
  <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/Javascript/javascript1.svg"/>
  <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/Firebase/firebase1.svg"/>
  <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/React/react3.svg"/>
  <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/TailwindCSS/tailwindcss3.svg"/>
  <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/NodeJS/nodejs2.svg"/>
  <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/Express/express1.svg"/>
  <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/MongoDB/mongodb2.svg"/>
  <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/Git/git2.svg"/>
  <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/Github/github2.svg"/>
</p>

---

## 🧩 What I Bring  

- 🧱 Ability to build **end-to-end systems (Frontend + Backend)**
- 🎨 Strong focus on **UI/UX + responsive design**
- ⚙️ Experience with **real-world workflows (auth, payments, dashboards)**
- 📦 Understanding of **state management & API architecture**
- 🚀 Continuous learner transitioning into **Data Analytics & scalable systems**

---

## 🔗 Connect With Me  

<p>
  <a href="https://linkedin.com/in/rohitsriv28">
    <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/LinkedIn/linkedin2.svg"/>
  </a>
  <a href="https://github.com/rohitsriv28">
    <img src="https://ziadoua.github.io/m3-Markdown-Badges/badges/Github/github2.svg"/>
  </a>
</p>

---

## 📊 GitHub Stats  

<div align="center">
  <img src="https://streak-stats.demolab.com?user=rohitsriv28&theme=dark" height="200"/>
</div>

---
